...
| "Save to Digest" bookmarklet fails with "Failed to fetch" | Bug | As a user, I want the bookmarklet to save YouTube videos/articles from any page so I don't have to copy-paste the URL back into the app. Currently fails on YouTube (https) with a generic "Failed to fetch" alert. Likely cause: `bookmarklet_origin` ([main.py:564](src/main.py#L564)) is derived from the request's own scheme/host, and `scripts/start.ps1` serves the app over plain `http://localhost:8001` — an https page (youtube.com) fetching an http:// endpoint is blocked by the browser as mixed content, which surfaces as this same generic error. Not yet confirmed by reproducing/inspecting the browser console. | Quick |
